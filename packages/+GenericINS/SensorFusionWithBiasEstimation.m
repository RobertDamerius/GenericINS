% SensorFusionWithBiasEstimation.m
% 
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
%                                    The class has been renamed to SensorFusionWithBiasEstimation.
% 
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
classdef SensorFusionWithBiasEstimation < handle
    methods(Static)
        function code = GenerateFunction(functionName, positionMeasurements, velocityMeasurements, orientationMeasurements, numSOGMeasurements)
            %GenericINS.SensorFusionWithBiasEstimation.GenerateFunction Generate a MATLAB function that implements a complete INS algorithm for a specific sensor configuration.
            % 
            % PARAMETERS
            ...
            assert(0 == (sum(positionMeasurements < 1) + sum(positionMeasurements > 3)), 'GenericINS.SensorFusionWithBiasEstimation.GenerateFunction(): "positionMeasurements" must only contain integer values {1, 2, 3}!');
            assert(0 == (sum(velocityMeasurements < 1) + sum(velocityMeasurements > 3)), 'GenericINS.SensorFusionWithBiasEstimation.GenerateFunction(): "velocityMeasurements" must only contain integer values {1, 2, 3}!');
            assert(0 == (sum(orientationMeasurements < 1) + sum(orientationMeasurements > 3)), 'GenericINS.SensorFusionWithBiasEstimation.GenerateFunction(): "orientationMeasurements" must only contain integer values {1, 2, 3}!');
            assert(isscalar(numSOGMeasurements), 'GenericINS.SensorFusionWithBiasEstimation.GenerateFunction(): "numSOGMeasurements" must be scalar!');
            ...
            docs = [docs '    %% This function was automatically generated using the command GenericINS.SensorFusionWithBiasEstimation.GenerateFunction(''' functionName ''', [' num2str(positionMeasurements) '], [' num2str(velocityMeasurements) '], [' num2str(orientationMeasurements) '], ' num2str(numSOGMeasurements) ').\n'];
            docs = [docs '    %% \n'];
            ...
            code = [code '    persistent ins; if(isempty(ins)), ins = GenericINS.SensorFusionWithBiasEstimation(); end\n'];
            ...
            assert(-1 ~= fileID, ['GenericINS.SensorFusionWithBiasEstimation.GenerateFunction(): Could not open file "' filename '"!']);
            fprintf(fileID, code);
            fclose(fileID);
        end
    end
    methods
        function obj = SensorFusionWithBiasEstimation()
            % Initialize private properties
            obj.initialized = false;
            obj.firstPrediction = false;
            obj.x = zeros(GenericINS.SensorFusionWithBiasEstimation.DIM_L, 1);
            obj.x(7) = 1.0;
            obj.S = zeros(GenericINS.SensorFusionWithBiasEstimation.DIM_LS);
            obj.Xi = zeros(GenericINS.SensorFusionWithBiasEstimation.DIM_L, GenericINS.SensorFusionWithBiasEstimation.NUM_SP);
            obj.Xi(7,:) = ones(1, GenericINS.SensorFusionWithBiasEstimation.NUM_SP);
            obj.dX = zeros(GenericINS.SensorFusionWithBiasEstimation.DIM_XS, GenericINS.SensorFusionWithBiasEstimation.NUM_SP);
            obj.spU2D = false;
            obj.gyrRaw = zeros(3, 1);
            obj.dcmIMUBody2Sensor = eye(3);
            obj.posIMUBody2Sensor = zeros(3, 1);
            obj.w0 = 0.1;
            obj.wi = (1.0 - obj.w0) / (double(GenericINS.SensorFusionWithBiasEstimation.DIM_LS) + 1.0);
            obj.srw0 = sqrt(obj.w0);
            obj.srwi = sqrt(obj.wi);

            % Initialize vector sequence for weighting matrix Z
            obj.Z = zeros(GenericINS.SensorFusionWithBiasEstimation.DIM_LS, GenericINS.SensorFusionWithBiasEstimation.NUM_SP);
            obj.Z(1,3) = 1.0 / sqrt(2.0 * obj.wi);
            obj.Z(1,2) = -obj.Z(1,3);

            % Expand vector sequence
            for j = int32(2):GenericINS.SensorFusionWithBiasEstimation.DIM_LS
        ...
        function Initialize(obj, initialPositionLLA, initialVelocityUVW, initialOrientationRollPitchYaw, initialBiasAcc, initialBiasGyr, initialStdPositionLLA, initialStdVelocityUVW, initialStdOrientationVector, initialStdBiasAcc, initialStdBiasGyr, stdAcc, stdGyr, stdAccBias, stdGyrBias)
            %GenericINS.SensorFusionWithBiasEstimation.Initialize Initialize or re-initialize the INS.
            % 
            % PARAMETERS
            ...
            assert((3 == size(initialPositionLLA,1)) && (1 == size(initialPositionLLA,2)) && isa(initialPositionLLA, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Initialize(): "initialPositionLLA" must be a 3-by-1 vector of type double!');
            assert((3 == size(initialVelocityUVW,1)) && (1 == size(initialVelocityUVW,2)) && isa(initialVelocityUVW, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Initialize(): "initialVelocityUVW" must be a 3-by-1 vector of type double!');
            assert((3 == size(initialOrientationRollPitchYaw,1)) && (1 == size(initialOrientationRollPitchYaw,2)) && isa(initialOrientationRollPitchYaw, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Initialize(): "initialOrientationRollPitchYaw" must be a 3-by-1 vector of type double!');
            assert((3 == size(initialBiasAcc,1)) && (1 == size(initialBiasAcc,2)) && isa(initialBiasAcc, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Initialize(): "initialBiasAcc" must be a 3-by-1 vector of type double!');
            assert((3 == size(initialBiasGyr,1)) && (1 == size(initialBiasGyr,2)) && isa(initialBiasGyr, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Initialize(): "initialBiasGyr" must be a 3-by-1 vector of type double!');
            assert((3 == size(initialStdPositionLLA,1)) && (1 == size(initialStdPositionLLA,2)) && isa(initialStdPositionLLA, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Initialize(): "initialStdPositionLLA" must be a 3-by-1 vector of type double!');
            assert((3 == size(initialStdVelocityUVW,1)) && (1 == size(initialStdVelocityUVW,2)) && isa(initialStdVelocityUVW, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Initialize(): "initialStdVelocityUVW" must be a 3-by-1 vector of type double!');
            assert((3 == size(initialStdOrientationVector,1)) && (1 == size(initialStdOrientationVector,2)) && isa(initialStdOrientationVector, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Initialize(): "initialStdOrientationVector" must be a 3-by-1 vector of type double!');
            assert((3 == size(initialStdBiasAcc,1)) && (1 == size(initialStdBiasAcc,2)) && isa(initialStdBiasAcc, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Initialize(): "initialStdBiasAcc" must be a 3-by-1 vector of type double!');
            assert((3 == size(initialStdBiasGyr,1)) && (1 == size(initialStdBiasGyr,2)) && isa(initialStdBiasGyr, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Initialize(): "initialStdBiasGyr" must be a 3-by-1 vector of type double!');
            assert((3 == size(stdAcc,1)) && (1 == size(stdAcc,2)) && isa(stdAcc, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Initialize(): "stdAcc" must be a 3-by-1 vector of type double!');
            assert((3 == size(stdGyr,1)) && (1 == size(stdGyr,2)) && isa(stdGyr, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Initialize(): "stdGyr" must be a 3-by-1 vector of type double!');
            assert((3 == size(stdAccBias,1)) && (1 == size(stdAccBias,2)) && isa(stdAccBias, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Initialize(): "stdAccBias" must be a 3-by-1 vector of type double!');
            assert((3 == size(stdGyrBias,1)) && (1 == size(stdGyrBias,2)) && isa(stdGyrBias, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Initialize(): "stdGyrBias" must be a 3-by-1 vector of type double!');

            % Initial position
            [lat, lon] = GenericINS.SensorFusionWithBiasEstimation.LatLon(initialPositionLLA(1), initialPositionLLA(2));
            obj.x(1) = lat;
            obj.x(2) = lon;
            ...
            % Initial velocity
            DCM_b2n = GenericINS.SensorFusionWithBiasEstimation.Cb2n(obj.x(7:10));
        ...
        function ResetInertialStandardDeviation(obj, stdAcc, stdGyr, stdAccBias, stdGyrBias)
            %GenericINS.SensorFusionWithBiasEstimation.ResetInertialStandardDeviation Reset the standard deviations for the inertial sensors.
            % 
            % PARAMETERS
            ...
            assert((3 == size(stdAcc,1)) && (1 == size(stdAcc,2)) && isa(stdAcc, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Predict(): "stdAcc" must be a 3-by-1 vector of type double!');
            assert((3 == size(stdGyr,1)) && (1 == size(stdGyr,2)) && isa(stdGyr, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Predict(): "stdGyr" must be a 3-by-1 vector of type double!');
            assert((3 == size(stdAccBias,1)) && (1 == size(stdAccBias,2)) && isa(stdAccBias, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Predict(): "stdAccBias" must be a 3-by-1 vector of type double!');
            assert((3 == size(stdGyrBias,1)) && (1 == size(stdGyrBias,2)) && isa(stdGyrBias, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Predict(): "stdGyrBias" must be a 3-by-1 vector of type double!');
            obj.S((GenericINS.SensorFusionWithBiasEstimation.DIM_XS + int32(1)):end,(GenericINS.SensorFusionWithBiasEstimation.DIM_XS + int32(1)):end) = diag([stdAcc; stdGyr; stdAccBias; stdGyrBias]);
        end
        function Predict(obj, imuData, sampletime, dcmIMUBody2Sensor, posIMUBody2Sensor)
            %GenericINS.SensorFusionWithBiasEstimation.Predict Perform a prediction step using inertial measurements. The prediction is only calculated if the INS is initialized.
            % 
            % PARAMETERS
            ...
            assert((6 == size(imuData,1)) && (1 == size(imuData,2)) && isa(imuData, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Predict(): "imuData" must be a 6-by-1 vector of type double!');
            assert(isscalar(sampletime) && isa(sampletime, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Predict(): "sampletime" must be a scalar of type double!');
            assert((3 == size(dcmIMUBody2Sensor,1)) && (3 == size(dcmIMUBody2Sensor,2)) && isa(dcmIMUBody2Sensor, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Predict(): "dcmIMUBody2Sensor" must be a 3-by-3 matrix of type double!');
            assert((3 == size(posIMUBody2Sensor,1)) && (1 == size(posIMUBody2Sensor,2)) && isa(posIMUBody2Sensor, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.Predict(): "posIMUBody2Sensor" must be a 3-by-1 vector of type double!');
            ...
            for i = int32(1):GenericINS.SensorFusionWithBiasEstimation.NUM_SP
                state = obj.Xi(1:GenericINS.SensorFusionWithBiasEstimation.DIM_X,i);
                w = obj.Xi((GenericINS.SensorFusionWithBiasEstimation.DIM_X+int32(1)):(GenericINS.SensorFusionWithBiasEstimation.DIM_X + GenericINS.SensorFusionWithBiasEstimation.DIM_W),i);
                obj.Xi(1:GenericINS.SensorFusionWithBiasEstimation.DIM_X,i) = GenericINS.SensorFusionWithBiasEstimation.ProcessModelEuler(state, w, imuData, sampletime, dcmIMUBody2Sensor);
            end
            ...
            [~, Sx_UPPER] = qr((obj.srwi*obj.dX(:,2:GenericINS.SensorFusionWithBiasEstimation.NUM_SP))', 0);
            Sx_UPPER = cholupdate(Sx_UPPER, obj.srw0*obj.dX(:,1), '+');
            obj.S(int32(1):GenericINS.SensorFusionWithBiasEstimation.DIM_XS,int32(1):GenericINS.SensorFusionWithBiasEstimation.DIM_XS) = Sx_UPPER';
        ...
        function UpdatePosition3D(obj, measurementLatLonAlt, stdNorthEastDown, posBody2Sensor)
            %GenericINS.SensorFusionWithBiasEstimation.UpdatePosition3D Perform an observation step for 3D geographic position data. The update is only calculated if the INS is initialized.
            % 
            % PARAMETERS
            ...
            assert((3 == size(measurementLatLonAlt,1)) && (1 == size(measurementLatLonAlt,2)) && isa(measurementLatLonAlt, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdatePosition3D(): "measurementLatLonAlt" must be a 3-by-1 vector of type double!');
            assert((3 == size(stdNorthEastDown,1)) && (1 == size(stdNorthEastDown,2)) && isa(stdNorthEastDown, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdatePosition3D(): "stdNorthEastDown" must be a 3-by-1 vector of type double!');
            assert((3 == size(posBody2Sensor,1)) && (1 == size(posBody2Sensor,2)) && isa(posBody2Sensor, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdatePosition3D(): "posBody2Sensor" must be a 3-by-1 vector of type double!');
        ...
        function UpdatePosition2D(obj, measurementLatLon, stdNorthEast, posBody2Sensor)
            %GenericINS.SensorFusionWithBiasEstimation.UpdatePosition2D Perform an observation step for 2D geographic position data. The update is only calculated if the INS is initialized.
            % 
            % PARAMETERS
            % measurementLatLon ... The measurement vector [lat (rad); lon (rad)].
            % stdNorthEast      ... The standard deviation in meters, [north; east].
            % posBody2Sensor    ... Position of sensor frame origin w.r.t. body frame origin in body frame coordinates.
            assert((2 == size(measurementLatLon,1)) && (1 == size(measurementLatLon,2)) && isa(measurementLatLon, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdatePosition2D(): "measurementLatLon" must be a 2-by-1 vector of type double!');
            assert((2 == size(stdNorthEast,1)) && (1 == size(stdNorthEast,2)) && isa(stdNorthEast, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdatePosition2D(): "stdNorthEast" must be a 2-by-1 vector of type double!');
            assert((3 == size(posBody2Sensor,1)) && (1 == size(posBody2Sensor,2)) && isa(posBody2Sensor, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdatePosition2D(): "posBody2Sensor" must be a 3-by-1 vector of type double!');
        ...
        function UpdatePosition1D(obj, measurementAlt, stdDown, posBody2Sensor)
            %GenericINS.SensorFusionWithBiasEstimation.UpdatePosition1D Perform an observation step for 1D geographic position data. The update is only calculated if the INS is initialized.
            % 
            % PARAMETERS
            % measurementAlt ... The measurement scalar [alt (m, positive upwards)].
            % stdDown        ... The standard deviation in meters, [down].
            % posBody2Sensor ... Position of sensor frame origin w.r.t. body frame origin in body frame coordinates.
            assert(isscalar(measurementAlt) && isa(measurementAlt, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdatePosition1D(): "measurementAlt" must be a scalar of type double!');
            assert(isscalar(stdDown) && isa(stdDown, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdatePosition1D(): "stdDown" must be a scalar of type double!');
            assert((3 == size(posBody2Sensor,1)) && (1 == size(posBody2Sensor,2)) && isa(posBody2Sensor, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdatePosition1D(): "posBody2Sensor" must be a 3-by-1 vector of type double!');
        ...
        function UpdateVelocity3D(obj, measurementXYZ, stdXYZ, posBody2Sensor, dcmBody2Sensor)
            %GenericINS.SensorFusionWithBiasEstimation.UpdateVelocity3D Perform an observation step for 3D velocity data. The update is only calculated if the INS is initialized.
            % 
            % PARAMETERS
            ...
            assert((3 == size(measurementXYZ,1)) && (1 == size(measurementXYZ,2)) && isa(measurementXYZ, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateVelocity3D(): "measurementXYZ" must be a 3-by-1 vector of type double!');
            assert((3 == size(stdXYZ,1)) && (1 == size(stdXYZ,2)) && isa(stdXYZ, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateVelocity3D(): "stdXYZ" must be a 3-by-1 vector of type double!');
            assert((3 == size(posBody2Sensor,1)) && (1 == size(posBody2Sensor,2)) && isa(posBody2Sensor, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateVelocity3D(): "posBody2Sensor" must be a 3-by-1 vector of type double!');
            assert((3 == size(dcmBody2Sensor,1)) && (3 == size(dcmBody2Sensor,2)) && isa(dcmBody2Sensor, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateVelocity3D(): "dcmBody2Sensor" must be a 3-by-3 matrix of type double!');
        ...
        function UpdateVelocity2D(obj, measurementXY, stdXY, posBody2Sensor, dcmBody2Sensor)
            %GenericINS.SensorFusionWithBiasEstimation.UpdateVelocity2D Perform an observation step for 2D velocity data. The update is only calculated if the INS is initialized.
            % 
            % PARAMETERS
            ...
            % posBody2Sensor ... Position of sensor frame origin w.r.t. body frame origin in body frame coordinates.
            % dcmBody2Sensor ... Direction cosine matrix to rotate vectors from body frame to sensor frame.
            assert((2 == size(measurementXY,1)) && (1 == size(measurementXY,2)) && isa(measurementXY, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateVelocity2D(): "measurementXY" must be a 2-by-1 vector of type double!');
            assert((2 == size(stdXY,1)) && (1 == size(stdXY,2)) && isa(stdXY, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateVelocity2D(): "stdXY" must be a 2-by-1 vector of type double!');
            assert((3 == size(posBody2Sensor,1)) && (1 == size(posBody2Sensor,2)) && isa(posBody2Sensor, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateVelocity2D(): "posBody2Sensor" must be a 3-by-1 vector of type double!');
            assert((3 == size(dcmBody2Sensor,1)) && (3 == size(dcmBody2Sensor,2)) && isa(dcmBody2Sensor, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateVelocity2D(): "dcmBody2Sensor" must be a 3-by-3 matrix of type double!');
        ...
        function UpdateVelocity1D(obj, measurementZ, stdZ, posBody2Sensor, dcmBody2Sensor)
            %GenericINS.SensorFusionWithBiasEstimation.UpdateVelocity1D Perform an observation step for 1D velocity data. The update is only calculated if the INS is initialized.
            % 
            % PARAMETERS
            ...
            % posBody2Sensor ... Position of sensor frame origin w.r.t. body frame origin in body frame coordinates.
            % dcmBody2Sensor ... Direction cosine matrix to rotate vectors from body frame to sensor frame.
            assert(isscalar(measurementZ) && isa(measurementZ, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateVelocity1D(): "measurementZ" must be a scalar of type double!');
            assert(isscalar(stdZ) && isa(stdZ, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateVelocity1D(): "stdZ" must be a scalar of type double!');
            assert((3 == size(posBody2Sensor,1)) && (1 == size(posBody2Sensor,2)) && isa(posBody2Sensor, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateVelocity1D(): "posBody2Sensor" must be a 3-by-1 vector of type double!');
            assert((3 == size(dcmBody2Sensor,1)) && (3 == size(dcmBody2Sensor,2)) && isa(dcmBody2Sensor, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateVelocity1D(): "dcmBody2Sensor" must be a 3-by-3 matrix of type double!');
        ...
        function UpdateSOG(obj, measurementSOG, stdSOG, posBody2Sensor)
            %GenericINS.SensorFusionWithBiasEstimation.UpdateSOG Perform an observation step for speed-over-ground data. The update is only calculated if the INS is initialized.
            % 
            % PARAMETERS
            % measurementSOG ... The measurement scalar, [sog (m/s)].
            % stdSOG         ... The standard deviation, [sog (m/s)].
            % posBody2Sensor ... Position of sensor frame origin w.r.t. body frame origin in body frame coordinates.
            assert(isscalar(measurementSOG) && isa(measurementSOG, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateSOG(): "measurementSOG" must be a scalar of type double!');
            assert(isscalar(stdSOG) && isa(stdSOG, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateSOG(): "stdSOG" must be a scalar of type double!');
            assert((3 == size(posBody2Sensor,1)) && (1 == size(posBody2Sensor,2)) && isa(posBody2Sensor, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateSOG(): "posBody2Sensor" must be a 3-by-1 vector of type double!');
        ...
        function UpdateOrientation3D(obj, measurementRollPitchYaw, stdRollPitchYaw, dcmBody2Sensor)
            %GenericINS.SensorFusionWithBiasEstimation.UpdateOrientation3D Perform an observation step for 3D orientation data. The update is only calculated if the INS is initialized.
            % 
            % PARAMETERS
            ...
            % dcmBody2Sensor          ... Direction cosine matrix to rotate vectors from body frame to sensor frame.
            assert((3 == size(measurementRollPitchYaw,1)) && (1 == size(measurementRollPitchYaw,2)) && isa(measurementRollPitchYaw, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateOrientation3D(): "measurementRollPitchYaw" must be a 3-by-1 vector of type double!');
            assert((3 == size(stdRollPitchYaw,1)) && (1 == size(stdRollPitchYaw,2)) && isa(stdRollPitchYaw, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateOrientation3D(): "stdRollPitchYaw" must be a 3-by-1 vector of type double!');
            assert((3 == size(dcmBody2Sensor,1)) && (3 == size(dcmBody2Sensor,2)) && isa(dcmBody2Sensor, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateOrientation3D(): "dcmBody2Sensor" must be a 3-by-3 matrix of type double!');
        ...
        function UpdateOrientation2D(obj, measurementRollPitch, stdRollPitch, dcmBody2Sensor)
            %GenericINS.SensorFusionWithBiasEstimation.UpdateOrientation2D Perform an observation step for 2D orientation data. The update is only calculated if the INS is initialized.
            % 
            % PARAMETERS
            % measurementRollPitch ... The measurement vector, [roll (rad); pitch (rad)].
            % stdRollPitch         ... The standard deviation, [roll (rad); pitch (rad)].
            % dcmBody2Sensor       ... Direction cosine matrix to rotate vectors from body frame to sensor frame.
            assert((2 == size(measurementRollPitch,1)) && (1 == size(measurementRollPitch,2)) && isa(measurementRollPitch, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateOrientation2D(): "measurementRollPitch" must be a 2-by-1 vector of type double!');
            assert((2 == size(stdRollPitch,1)) && (1 == size(stdRollPitch,2)) && isa(stdRollPitch, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateOrientation2D(): "stdRollPitch" must be a 2-by-1 vector of type double!');
            assert((3 == size(dcmBody2Sensor,1)) && (3 == size(dcmBody2Sensor,2)) && isa(dcmBody2Sensor, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateOrientation2D(): "dcmBody2Sensor" must be a 3-by-3 matrix of type double!');
        ...
        function UpdateOrientation1D(obj, measurementYaw, stdYaw, dcmBody2Sensor)
            %GenericINS.SensorFusionWithBiasEstimation.UpdateOrientation1D Perform an observation step for 1D orientation data. The update is only calculated if the INS is initialized.
            % 
            % PARAMETERS
            % measurementYaw ... The measurement scalar, [yaw (rad)].
            % stdYaw         ... The standard deviation, [yaw (rad)].
            % dcmBody2Sensor ... Direction cosine matrix to rotate vectors from body frame to sensor frame.
            assert(isscalar(measurementYaw) && isa(measurementYaw, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateOrientation1D(): "measurementYaw" must be a scalar of type double!');
            assert(isscalar(stdYaw) && isa(stdYaw, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateOrientation1D(): "stdYaw" must be a scalar of type double!');
            assert((3 == size(dcmBody2Sensor,1)) && (3 == size(dcmBody2Sensor,2)) && isa(dcmBody2Sensor, 'double'), 'GenericINS.SensorFusionWithBiasEstimation.UpdateOrientation1D(): "dcmBody2Sensor" must be a 3-by-3 matrix of type double!');
        ...
        function [valid, positionLLA, orientationQuaternionWXYZ, orientationRollPitchYaw, velocityNED, velocityUVW, velocityPQR, courseOverGround, speedOverGround, angleOfAttack, sideSlipAngle, inertialSensorBias] = GetMotionState(obj)
            %GenericINS.SensorFusionWithBiasEstimation.GetMotionState Get the motion state.
            % 
            % RETURN
            ...
            % WGS84 properties
            [Rn, Re, ~] = GenericINS.SensorFusionWithBiasEstimation.WGS84(obj.x(1));

            % Orientation
            orientationQuaternionWXYZ = GenericINS.SensorFusionWithBiasEstimation.Normalize(obj.x(7:10));
            Cq = GenericINS.SensorFusionWithBiasEstimation.Cb2n(orientationQuaternionWXYZ);
            [roll, pitch, yaw] = GenericINS.SensorFusionWithBiasEstimation.EulerZYX(Cq);
            ...
            [lat, lon] = GenericINS.SensorFusionWithBiasEstimation.LatLon(positionLLA(1), positionLLA(2));
            positionLLA(1) = lat;
            positionLLA(2) = lon;

            % Rotational velocity (remove bias and align to body frame)
            w_ie = [GenericINS.SensorFusionWithBiasEstimation.OMEGA_EARTH * cos(obj.x(1)); 0; -GenericINS.SensorFusionWithBiasEstimation.OMEGA_EARTH * sin(obj.x(1))];
            ...
            % COG, SOG, AOA, SSA
            courseOverGround = GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle(atan2(velocityNED(2), velocityNED(1)));
            speedOverGround = sqrt(velocityNED(1:2)'*velocityNED(1:2));
            angleOfAttack = GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle(atan2(velocityNED(3), speedOverGround));
            sideSlipAngle = GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle(courseOverGround - yaw);

            % Inertial sensor bias
            inertialSensorBias = obj.x(11:16);
        end
        function [x, S] = GetInternalState(obj)
            %GenericINS.SensorFusionWithBiasEstimation.GetInternalState Get the internal state of the filter.
            % 
            % RETURN
            ...
            x = obj.x(int32(1):GenericINS.SensorFusionWithBiasEstimation.DIM_X);
            S = obj.S(int32(1):GenericINS.SensorFusionWithBiasEstimation.DIM_XS,int32(1):GenericINS.SensorFusionWithBiasEstimation.DIM_XS);
        end
        function MakeInvalid(obj)
            %GenericINS.SensorFusionWithBiasEstimation.MakeInvalid Make the filter status invalid. An initialization followed by at least one prediction step
    ...
    methods(Access=private)
        function GenerateSigmaPoints(obj)
            ...
            obj.Xi = repmat(obj.x, 1, GenericINS.SensorFusionWithBiasEstimation.NUM_SP);

            % For remaining sigma points S*Z is required
            SZ = obj.S * obj.Z(:,int32(2):GenericINS.SensorFusionWithBiasEstimation.NUM_SP);
            ...
            SZq = [SZ(1:6,:); zeros(4,GenericINS.SensorFusionWithBiasEstimation.NUM_SP - int32(1)); SZ(10:end,:)];
            SZq(7:10,:) = GenericINS.SensorFusionWithBiasEstimation.OV2Q(SZ(7:9,:));
            ...
            obj.Xi(7:10,2:end)   = GenericINS.SensorFusionWithBiasEstimation.Qdot(SZq(7:10,:), obj.Xi(7:10,1)); 
        ...
        function CalculateDX(obj)
            ...
            obj.Xi(2,:) = GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle(obj.Xi(2,:) - repmat(pivotAngle, 1, GenericINS.SensorFusionWithBiasEstimation.NUM_SP));

            % Calculate weighted mean of sigma points (barycentric mean for quaternion part)
            obj.x(1:GenericINS.SensorFusionWithBiasEstimation.DIM_X) = obj.w0 * obj.Xi(1:GenericINS.SensorFusionWithBiasEstimation.DIM_X,1) + obj.wi * sum(obj.Xi(1:GenericINS.SensorFusionWithBiasEstimation.DIM_X,2:GenericINS.SensorFusionWithBiasEstimation.NUM_SP), 2);
            obj.x(7:10) = GenericINS.SensorFusionWithBiasEstimation.Normalize(obj.x(7:10));

            % Calculate dX = Xi - x
            % Special treatment for angle and quaternion. Xi - x: We need the inverse quaternion because of -x.
            q_inv = [obj.x(7); -obj.x(8:10)];
            dXq = GenericINS.SensorFusionWithBiasEstimation.Qdot(obj.Xi(7:10,:), q_inv);
            obj.dX = [obj.Xi(1,:) - repmat(obj.x(1), 1, GenericINS.SensorFusionWithBiasEstimation.NUM_SP); ...
                  GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle(obj.Xi(2,:) - repmat(obj.x(2), 1, GenericINS.SensorFusionWithBiasEstimation.NUM_SP)); ...
                  obj.Xi(3:6,:) - repmat(obj.x(3:6), 1, GenericINS.SensorFusionWithBiasEstimation.NUM_SP); ...
                  GenericINS.SensorFusionWithBiasEstimation.Q2OV(dXq); ...
                  obj.Xi(11:GenericINS.SensorFusionWithBiasEstimation.DIM_X,:) - repmat(obj.x(11:GenericINS.SensorFusionWithBiasEstimation.DIM_X), 1, GenericINS.SensorFusionWithBiasEstimation.NUM_SP)];

            % Add pivot angle to state (and also sigma-points for an upcomming measurement update)
            obj.x(2) = GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle(obj.x(2) + pivotAngle);
            obj.Xi(2,:) = GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle(obj.Xi(2,:) + repmat(pivotAngle, 1, GenericINS.SensorFusionWithBiasEstimation.NUM_SP));
        end
        function UpdatePosition(obj, yDim, measurement, stdMeasurement, posBody2Sensor)
            ...
            yDim = int32(yDim);
            Y = zeros(yDim, GenericINS.SensorFusionWithBiasEstimation.NUM_SP);
            switch(yDim)
                case 1
                    for i = int32(1):GenericINS.SensorFusionWithBiasEstimation.NUM_SP
                        state = obj.Xi(1:GenericINS.SensorFusionWithBiasEstimation.DIM_X,i);
                        y = GenericINS.SensorFusionWithBiasEstimation.SensorModelPosition(state, obj.posIMUBody2Sensor, posBody2Sensor);
                        Y(:,i) = y(3);
                    end
                case 2
                    for i = int32(1):GenericINS.SensorFusionWithBiasEstimation.NUM_SP
                ...
                case 3
                    for i = int32(1):GenericINS.SensorFusionWithBiasEstimation.NUM_SP
                        state = obj.Xi(1:GenericINS.SensorFusionWithBiasEstimation.DIM_X,i);
                        Y(:,i) = GenericINS.SensorFusionWithBiasEstimation.SensorModelPosition(state, obj.posIMUBody2Sensor, posBody2Sensor);
                    end
            end
            ...
            if(yDim > int32(1))
                pivotAngle = Y(2,1);
                Y(2,:) = GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle(Y(2,:) - repmat(pivotAngle, 1, GenericINS.SensorFusionWithBiasEstimation.NUM_SP));
            end

            % Calculate weighted mean of sigma-points
            ym = obj.w0 * Y(:,1) + obj.wi * sum(Y(:,int32(2):GenericINS.SensorFusionWithBiasEstimation.NUM_SP), 2);

            % Calculate dY = Y - ym
            dY = (Y - repmat(ym, 1, GenericINS.SensorFusionWithBiasEstimation.NUM_SP));

            % Add pivot angle to mean value
            if(yDim > int32(1))
                ym(2) = GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle(ym(2) + pivotAngle);
            end

            % Calculate the innovation
            innovation = measurement - ym;
            if(yDim > int32(1))
                innovation(2) = GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle(innovation(2));
            end
            ...
            [~, Sy_UPPER] = qr([(obj.srwi * dY(:,2:GenericINS.SensorFusionWithBiasEstimation.NUM_SP)) sqrtR]', 0);
            ...
            Pxy = obj.w0 * obj.dX(:,1) * dY(:,1)' + obj.wi * obj.dX(:,int32(2):GenericINS.SensorFusionWithBiasEstimation.NUM_SP) * dY(:,int32(2):GenericINS.SensorFusionWithBiasEstimation.NUM_SP)';
            ...
            obj.x(2) = GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle(obj.x(2));
            obj.x(7:10) = GenericINS.SensorFusionWithBiasEstimation.Qdot(GenericINS.SensorFusionWithBiasEstimation.OV2Q(dx(7:9)), obj.x(7:10));
            obj.x(11:GenericINS.SensorFusionWithBiasEstimation.DIM_X) = obj.x(11:GenericINS.SensorFusionWithBiasEstimation.DIM_X) + dx(10:end);
            ...
            Sx_UPPER = obj.S(int32(1):GenericINS.SensorFusionWithBiasEstimation.DIM_XS,int32(1):GenericINS.SensorFusionWithBiasEstimation.DIM_XS)';
            U = K * Sy;
            for j = 1:yDim
        ...
        function UpdateVelocity(obj, yDim, measurement, stdMeasurement, posBody2Sensor, dcmBody2Sensor)
            ...
            switch(yDim)
                case 1
                    for i = int32(1):GenericINS.SensorFusionWithBiasEstimation.NUM_SP
                        state = obj.Xi(1:GenericINS.SensorFusionWithBiasEstimation.DIM_X,i);
                        y = GenericINS.SensorFusionWithBiasEstimation.SensorModelVelocity(state, obj.gyrRaw, obj.posIMUBody2Sensor, obj.dcmIMUBody2Sensor, posBody2Sensor, dcmBody2Sensor);
                        Y(:,i) = y(3);
                    end
                case 2
                    for i = int32(1):GenericINS.SensorFusionWithBiasEstimation.NUM_SP
                ...
                case 3
                    for i = int32(1):GenericINS.SensorFusionWithBiasEstimation.NUM_SP
                        state = obj.Xi(1:GenericINS.SensorFusionWithBiasEstimation.DIM_X,i);
                        Y(:,i) = GenericINS.SensorFusionWithBiasEstimation.SensorModelVelocity(state, obj.gyrRaw, obj.posIMUBody2Sensor, obj.dcmIMUBody2Sensor, posBody2Sensor, dcmBody2Sensor);
                    end
            end
        ...
        function UpdateSpeedOverGround(obj, measurement, stdMeasurement, posBody2Sensor)
            ...
            % Propagate through measurement model
            Y = zeros(1, GenericINS.SensorFusionWithBiasEstimation.NUM_SP);
            for i = int32(1):GenericINS.SensorFusionWithBiasEstimation.NUM_SP
                state = obj.Xi(1:GenericINS.SensorFusionWithBiasEstimation.DIM_X,i);
                Y(:,i) = GenericINS.SensorFusionWithBiasEstimation.SensorModelSpeedOverGround(state, obj.gyrRaw, obj.posIMUBody2Sensor, obj.dcmIMUBody2Sensor, posBody2Sensor);
            end
        ...
        function UpdateOrientation(obj, yDim, measurement, stdMeasurement, dcmBody2Sensor)
            ...
            switch(yDim)
                case 1
                    for i = int32(1):GenericINS.SensorFusionWithBiasEstimation.NUM_SP
                        state = obj.Xi(1:GenericINS.SensorFusionWithBiasEstimation.DIM_X,i);
                        y = GenericINS.SensorFusionWithBiasEstimation.SensorModelOrientation(state, dcmBody2Sensor);
                        Y(:,i) = y(3);
                    end
                case 2
                    for i = int32(1):GenericINS.SensorFusionWithBiasEstimation.NUM_SP
                ...
                case 3
                    for i = int32(1):GenericINS.SensorFusionWithBiasEstimation.NUM_SP
                        state = obj.Xi(1:GenericINS.SensorFusionWithBiasEstimation.DIM_X,i);
                        Y(:,i) = GenericINS.SensorFusionWithBiasEstimation.SensorModelOrientation(state, dcmBody2Sensor);
                    end
            end

            % Get pivot angle from 0-th Y sigma-point and remove this angle from all sigma-points
            pivotAngle = Y(:,1);
            Y = GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle(Y - repmat(pivotAngle, 1, GenericINS.SensorFusionWithBiasEstimation.NUM_SP));
            ...
            % Add pivot angle to mean value
            ym = GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle(ym + pivotAngle);

            % Calculate the innovation
            innovation = GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle(measurement - ym);
    ...
    methods(Static,Access=private)
        function y = Normalize(u)
            %GenericINS.SensorFusionWithBiasEstimation.Normalize Safe normalization.
            % 
            % PARAMETERS
        ...
        function y = SymmetricalAngle(x)
            %GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle Convert a given angle x [rad] to an output angle y [rad] with y being in range [-pi, +pi).
            % 
            % PARAMETERS
        ...
        function Q = OV2Q(OV)
            %GenericINS.SensorFusionWithBiasEstimation.OV2Q Convert an orientation vector to a unit quaternion.
            % 
            % PARAMETERS
        ...
        function OV = Q2OV(Q)
            %GenericINS.SensorFusionWithBiasEstimation.Q2OV Convert a unit quaternion to an orientation vector. The input quaternion will be normalized.
            % 
            % PARAMETERS
        ...
        function Y = Qdot(Q, r)
            %GenericINS.SensorFusionWithBiasEstimation.Qdot Calculate the quaternion multiplication for two unit quaternions Q and r. The output quaternion will be a unit quaternion.
            % 
            % PARAMETERS
        ...
        function C = Cb2n(q)
            %GenericINS.SensorFusionWithBiasEstimation.Cb2n Calculate the rotation matrix for a given unit quaternion.
            % 
            % PARAMETERS
        ...
        function [roll, pitch, yaw] = EulerZYX(R)
            %GenericINS.SensorFusionWithBiasEstimation.EulerZYX Calculate the corresponding euler angles (zyx-convention) for a given rotation matrix (b-frame to n-frame).
            % 
            % PARAMETERS
            ...
            R = reshape(R, [3 3]);
            roll = GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle(atan2(R(3,2), R(3,3)));
            pitch = -real(asin(min(max(R(3,1),-1.0),1.0)));
            yaw = GenericINS.SensorFusionWithBiasEstimation.SymmetricalAngle(atan2(R(2,1), R(1,1)));
        end
        function [Rn, Re, R0] = WGS84(latitude)
            %GenericINS.SensorFusionWithBiasEstimation.WGS84 Get information about the WGS84 earth reference model for a given latitude in radians.
            % 
            % PARAMETERS
        ...
        function g = Gravity(altitude, latitude, R0)
            %GenericINS.SensorFusionWithBiasEstimation.Gravity Compute the local gravity g [m/s^2] for a given position using the WGS84 earth reference model.
            % 
            % PARAMETERS
        ...
        function [lat, lon] = LatLon(phi, lambda)
            %GenericINS.SensorFusionWithBiasEstimation.LatLon Convert phi [rad] and lambda [rad] to latitude [rad] and longitude [rad] with latitude being in range [-pi/2, +pi/2] and longitude being in range [-pi; +pi).
            % 
            % PARAMETERS
        ...
        function [lat, lon, alt] = NED2LLA(N, E, D, originLat, originLon, originAlt)
            %GenericINS.SensorFusionWithBiasEstimation.NED2LLA Convert local NED frame coordinates to LLA coordinates (WGS84).
            % 
            % PARAMETERS
        ...
        function xOut = ProcessModelEuler(x, w, u, Ts, dcmIMUBody2Sensor)
            ...
            % WGS84 properties
            [Rn, Re, R0] = GenericINS.SensorFusionWithBiasEstimation.WGS84(x(1));
            localGravity = GenericINS.SensorFusionWithBiasEstimation.Gravity(x(3), x(1), R0);
            ...
            x(7:10) = GenericINS.SensorFusionWithBiasEstimation.Normalize(x(7:10));
            DCM_b2n = GenericINS.SensorFusionWithBiasEstimation.Cb2n(x(7:10));
            DCM_n2b = DCM_b2n';
            ...
            [lat, lon] = GenericINS.SensorFusionWithBiasEstimation.LatLon(xOut(1), xOut(2));
            xOut(1) = lat;
            xOut(2) = lon;

            % Predict velocity
            w_ie = [GenericINS.SensorFusionWithBiasEstimation.OMEGA_EARTH * cos(x(1)); 0; -GenericINS.SensorFusionWithBiasEstimation.OMEGA_EARTH * sin(x(1))];
            ...
            xOut(7:10) = GenericINS.SensorFusionWithBiasEstimation.Normalize(xOut(7:10));
        ...
        function y = SensorModelPosition(x, posIMUBody2Sensor, posPOSBody2Sensor)
            ...
            q = GenericINS.SensorFusionWithBiasEstimation.Normalize(x(7:10));
            r_NED = GenericINS.SensorFusionWithBiasEstimation.Cb2n(q) * (posPOSBody2Sensor - posIMUBody2Sensor);

            % Transform from NED-frame to geographic coordinates (WGS84)
            [lat, lon, alt] = GenericINS.SensorFusionWithBiasEstimation.NED2LLA(r_NED(1),r_NED(2),r_NED(3),x(1),x(2),x(3));
            y = [lat; lon; alt];
        end
        function y = SensorModelVelocity(x, gyrRaw, posIMUBody2Sensor, dcmIMUBody2Sensor, posBody2Sensor, dcmBody2Sensor)
            ...
            DCM_b2n = GenericINS.SensorFusionWithBiasEstimation.Cb2n(GenericINS.SensorFusionWithBiasEstimation.Normalize(x(7:10)));
            DCM_n2b = DCM_b2n';

            % Angular rate of body with respect to the earth
            w_ie = GenericINS.SensorFusionWithBiasEstimation.OMEGA_EARTH * [cos(x(1)); 0; -sin(x(1))];
        ...
        function y = SensorModelOrientation(x, dcmBody2Sensor)
            ...
            DCM_S2N = GenericINS.SensorFusionWithBiasEstimation.Cb2n(q) * dcmBody2Sensor';
            [roll,pitch,yaw] = GenericINS.SensorFusionWithBiasEstimation.EulerZYX(DCM_S2N);
    ...
    properties(Constant,Access=private)
        ...
        DIM_XS = GenericINS.SensorFusionWithBiasEstimation.DIM_X - int32(1);                                          % Dimension of state vector (x) with respect to unvertainty S.
        DIM_W  = int32(12);                                                                                           % Dimension of process noise (w).
        DIM_L  = GenericINS.SensorFusionWithBiasEstimation.DIM_X + GenericINS.SensorFusionWithBiasEstimation.DIM_W;   % Dimension of augmented state vector.
        DIM_LS = GenericINS.SensorFusionWithBiasEstimation.DIM_XS + GenericINS.SensorFusionWithBiasEstimation.DIM_W;  % Dimension of augmented state vector with respect to uncertainty S.
        NUM_SP = GenericINS.SensorFusionWithBiasEstimation.DIM_LS + int32(2);                                         % Number of sigma-points.
